...
/**
 * OpenStackNovaProjectLdap supports the older ldap-based Openstack project assignment.
 *
 *  This file is preserver for transitional/migration purposes.  It should be removed once
 *   everything is using the new OpenStackNovaProject class.
 *
 * @file
 * @ingroup Extensions
 */

class OpenStackNovaProjectLdap {
	public $projectname;
	public $projectDN;
	public $projectInfo;
	public $roles;
	...
	function fetchProjectInfo( $refresh=true ) {
		...
		foreach ( self::$rolenames as $rolename ) {
			$this->roles[] = OpenStackNovaRoleLdap::getProjectRoleByName( $rolename, $this );
		}
	...
	function loadMembers() {
		...
		$key = wfMemcKey( 'openstackmanager', 'projectuidsandmembersldap', $this->projectname );
	...
	static function getProjectByName( $projectname ) {
		...
		$project = new OpenStackNovaProjectLdap( $projectname );
	 ...
	 *
	 * @static
	 * @return OpenStackNovaProjectLdap[]
	 */
		...
		if ( $result ) {
			...
			if ( $entries ) {
				...
				foreach ( $entries as $entry ) {
					$project = new OpenStackNovaProjectLdap( $entry['cn'][0], false );
					$projects[] = $project;
				}
			}
		}
	...
	static function createProject( $projectname ) {
		...
		if ( $success ) {
			foreach ( self::$rolenames as $rolename ) {
				OpenStackNovaRoleLdap::createRole( $rolename, $project );
		...
		OpenStackNovaProjectLdap::createServiceGroupOUs( $projectname );
